...
def login() -> None:
    ...
    checksum = bytes(
        token + username + token + hmd5 + token + data['ac_id'] + token + rsp_json['client_ip'] + token + data[
            'n'] + token + data['type'] + token + data['info'], 'utf-8')
...
def xencode(msg: str, key) -> str:
    def char_code_at(stri, index) -> int:
        return 0 if index >= len(stri) else ord(stri[index])

    def s(a: str, b: bool) -> list:
        c = len(a)
        v = []
    ...
    if len(sys.argv) == 2:
        ...
        if arg == 'login':
            if username == '' or password == '':
                print('Please set username and password in config.py first!')
            else:
                login()
        elif arg == 'logout':
            if username == '' or password == '':
